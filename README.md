# PBA-based Low-Fidelity Model, основанная на обобщенном методе трапеций
...
#### Burgers Equation (уравнение Бургерса)

**PDE:** ∂u/∂t + u · ∂u/∂x = ν · ∂²u/∂x²

| Параметр | Значение |
|----------|:--------:|
| Domain | x ∈ [−1, 1], t ∈ [0, 0.5] |
| ν | 0.01 |
| IC | u(x, 0) = −sin(πx) |
| BC | u(−1, t) = u(1, t) (периодические) |
| Exact | Cole-Hopf transformation (numerical integration) |

#### Reaction-Diffusion Equation (уравнение реакции-диффузии)
...
| D | 0.01 |
| r | 1.0 |
| IC | u(x, 0) = 0.5 · (1 + tanh(x / √(2D))) |
| BC | u(−1, t) = 0, u(1, t) = 1 (Dirichlet) |
| Exact | u(x, t) = 0.5 · (1 + tanh((x − vt) / √(2D))), v = √(Dr/2) |
...
**Key points:**
- На чистых данных Heat достигает +83.6% улучшения даже без дополнительных параметров — данные напрямую корректируют θ-сеть
- Для Wave комбинация time_gate+rhs_scale даёт максимальный эффект (+79%), масштабируя правую часть PDE
- С шумом результат θ-сети без дополнительных обучаемых параметров ухудшается
...
**Выводы:**
- Для каждой комбинации уравнение-корректировка, использовать отдельный, наиболее эффективный **learning rate**
...
![PDE Benchmark Comparison — Heat](plots/heat_eq_pde_training_self_benchmark.png)

| PDE | n_steps | n_iter | LR | θ₀ | Params | L2RE | RMSE/Mean Error | Final θ |
|-----|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Heat | 4 | 2 | 0.005 | 0.4 | 13 | 1.34e-02 | 3.10e-03 | 0.3073 ± 0.0503 |
...
- Финальные значения θ стабильны (небольшое стандартное отклонение), что говорит о согласованности θ-сети по пространству
...
- Для **нелинейных и жестких задач** (Burgers, Reaction-Diffusion) Classical PINN лучше использует данные благодаря бо́льшему числу свободных параметров
